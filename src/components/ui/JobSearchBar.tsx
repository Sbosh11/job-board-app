...
export default function JobSearchBar({
  ...
  const isKeywordControlled = keywordValue !== undefined;
  const isLocationControlled = locationValue !== undefined;

  const [uncontrolledKeyword, setUncontrolledKeyword] = useState("");
  const [uncontrolledLocation, setUncontrolledLocation] = useState("");

  const keyword = isKeywordControlled ? keywordValue : uncontrolledKeyword;
  const location = isLocationControlled ? locationValue : uncontrolledLocation;
  ...
  function setKeywordValue(value: string) {
    if (isKeywordControlled) {
      ...
      setUncontrolledKeyword(value);
    }
  }

  function setLocationValue(value: string) {
    if (isLocationControlled) {
      onLocationChange?.(value);
    } else {
      setUncontrolledLocation(value);
    }
  }
